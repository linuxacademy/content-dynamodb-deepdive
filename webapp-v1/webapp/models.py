...
class Track:
    ...
    def get_track(track_id):
        ...
        item = resp.get('Item')
        track = Track(item['id'], item['album_id'],
                      item['name'], item['length'], item['number'])
        return track
...
class Album:
    ...
    def __init__(self, id, sku, artist_id, title, year, format, price,
                 cover_art=None, tracks: List[Track] = []):
        self._id = id
        self._sku = sku
    ...
    def album_art(self):
        ...
        filename = os.path.join(filepath, f'{self.id}.jpg')
    ...
    def get_by_id(album_id: int):
        ...
        album = Album(int(item['id']), item['sku'], int(item['artist_id']), item['title'],
                      int(item['year']), item['format'], item['price'])
    ...
    def get_all(prev_evaluated_key=None, last_evaluated_key=None, per_page=9):
        ...
        for item in response['Items']:
            album = Album(item['id'], item['sku'], item['artist_id'],
                          item['title'], item['year'], item['format'],
                          item['price'], cover_art=b'', tracks=[])

            album.artist = Artist.get_by_id(
                album.artist_id, ignore_albums=True)
    ...
    def find_by_artist(artist_name, prev, last, per_page):
        ...
        albums, last, prev = Album.find_by_artist_ids(
            artist_ids, prev, last, per_page)

        return albums, last, prev

    @staticmethod
    def find_by_artist_ids(artist_ids: List[int], prev_evaluated_key, last_evaluated_key, per_page=9):
        ...
        for item in data:
            album = Album(int(item['id']), item['sku'], int(item['artist_id']),
                          item['title'], int(item['year']), item['format'],
                          item['price'])
            print(album)
            albums.append(album)
    ...
    def find_by_track(track_name):
        ...
        for item in data:
            track = Track(int(item['id']), int(
                item['album_id']), item['name'], item['length'], item['number'])
            tracks.append(track)
